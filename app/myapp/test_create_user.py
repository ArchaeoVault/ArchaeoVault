...
class CreateUserTests(TestCase):
    ...
    def test_create_user_missing_value(self):
        ...
        response = self.client.post(reverse('create_user_view'), data = {'first_name':'','last_name':'','email': '','password': '','confirm_password':''})
        self.assertEqual(response.status_code, 400)
    
    def test_create_user_invalid_email(self):
        
        response = self.client.post(reverse('create_user_view'), data = {'first_name':'test','last_name':'user','email':'temp','password':'password123','confirm_password':'password123'})
        self.assertEqual(response.status_code, 400)

    def test_mismatch_password(self):

        response = self.client.post(reverse('create_user_view'), data = {'first_name':'test','last_name':'user','email':'temp@email.com','password':'password123','confirm_password':'password12'})
        self.assertEqual(response.status_code,400)
